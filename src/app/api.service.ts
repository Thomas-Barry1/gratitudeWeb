...
export class ApiService {
  ...
  getList(): string {
    ...
    const returnObj = this.affirmations[this.current];
    if(this.current === this.affirmations.length-1){
      this.current = 0;
    }else{
  ...
  randomizeOrder(){
    for (var i = this.affirmations.length - 1; i > 0; i--) {
      var j = Math.floor(Math.random() * (i + 1));
      var temp = this.affirmations[i];
      this.affirmations[i] = this.affirmations[j];
      this.affirmations[j] = temp;
  }
  }
